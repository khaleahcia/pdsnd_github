...
def get_filters():
    ...
    month = get_input('\nWhich month do you want to filter the data by?\nChoose January, February, March, April, May, June, or all. ', ['all', 'january', 'february', 'march', 'april', 'may', 'june'])

    # Gets valid user input for day of week (all, monday, tuesday, ... sunday)
    day = get_input('\nWhich day of the week do you want to fliter by?\nChoose Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday, or all. ', ['all', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'])
...
def time_stats(df):
    ...
    print('\nThe most frequent day of the week to travel: ' + str(df['day_of_week'].mode()))
...
def user_stats(df, city):
    ...
    if city == 'washington':
        ...
        print('\nEarliest birth year: ' + str(df['Birth Year'].min(skipna=True)))
        print('\nMost recent birth year: ' + str(df['Birth Year'].max(skipna=True)))
